  ...
  return (
    <div className="space-y-8 pb-16">
      <div>
        ...
        <p className="text-sm text-muted mt-1">{userId}</p>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="stat-card">
              ...
              <span className="stat-value">{profile.avg_rating?.toFixed(1) || "N/A"}</span>
            </div>
            <div className="stat-card">
            ...
            <div className="stat-card">
              ...
              <span className="stat-value">{Object.keys(profile.genre_preferences).length}</span>
            </div>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card p-5">
              <h2 className="text-sm font-semibold text-primary mb-4">Genre Preferences</h2>
              <div className="space-y-2.5">
                {Object.entries(profile.genre_preferences).slice(0, 8).map(([genre, score]) => (
                  <div key={genre} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-secondary">{genre}</span>
                      <span className="text-muted">{((score as number) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-primary rounded-full transition-all"
                        style={{ width: `${((score as number) / maxGenreScore) * 100}%` }}
                      />
                    </div>
                  </div>
                ))}
            ...
            <div className="card p-5">
              <h2 className="text-sm font-semibold text-primary mb-4">Rating Distribution</h2>
              <div className="space-y-2">
                {[5, 4, 3, 2, 1].map((star) => {
                  const count = profile.rating_distribution?.[star.toString()] || 0;
                  const maxCount = Math.max(
                    ...Object.values(profile.rating_distribution as Record<string, number>),
                    1
                  );
                  return (
